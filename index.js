  ...
  switch (member) {
    ...
    case "Intern":
      let { school } = await inquirer.prompt(
        questions.item(member, "school", "school", validate.required)
      );
...
async function generateHTML() {
  let Template = {
    Main: await getHTMLModule("./HTMLtemplates/main.html"),
    Manager: await getHTMLModule("./HTMLtemplates/manager.html"),
    Engineer: await getHTMLModule("./HTMLtemplates/engineer.html"),
    Intern: await getHTMLModule("./HTMLtemplates/intern.html"),
  };
  ...
  let completeHTML = Template["Main"].replace(
    /{% employees %}/gi,
    employeesHTML
  );
...
// function to start the first part of the prompt
async function init() {
